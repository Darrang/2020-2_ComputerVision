...
int main(){
    ...
    while(1){
       ...
       HoughLines(left_dst,linesL,1,CV_PI / 180, 90, 0, 0, CV_PI/6, CV_PI/3); //왼쪽 직선 검출
       HoughLines(right_dst,linesR,1,CV_PI / 180, 90, 0, 0, (CV_PI/3) * 2, (CV_PI/6) * 5); //오른쪽 직선 검출     
        ...
        if(time > 0 && time < 5000 || time > 29000 && time < 31000){
            if(linesL.size() == 0 && linesR.size() == 0){
                putText(canny,format("Warning!: Lane departure"),Point(450,100),FONT_HERSHEY_SIMPLEX,1,Scalar(0,0,255),2);
            }
        }    
